...
public class LoginController {
	...
	public String getLogin(Model model,
		...
		if(member != null) { //member객체에 값이 들어있다면!
			session.setAttribute("loginSession", member);
			return "redirect:/"; //로그인 성공하면 메인페이지로 보내기
		} else { //일치하는정보가없다면
			model.addAttribute("error", "일치하는 아이디 비밀번호가 없습니다.");
			model.addAttribute("m", new Member());
			return "login"; //다시 로그인입력창으로 돌려보내기
		}
	...
	public String logout(HttpSession session) {
		session.invalidate(); // invalidate 무효시키다. 없던일로 만들다
		return "redirect:/";
	}
	/*
	 HttpSession session
