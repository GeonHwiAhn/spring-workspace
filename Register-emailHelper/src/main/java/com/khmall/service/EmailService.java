...
public class EmailService {
	...
	// 임의로 6자리를 생성하는 규칙
	/*
	 * Math.random() 0.0 ~ 1.0 미만의 임의의 소수(실수) 생성 예를들어 0.0 부터 0.9999 사이의 값을 반환하는 등 실수
	 * 생성 Math.random() 에 (90000) 곱하기를 해주면 0.0 부터 8999999.999 사이의 값을 반환 정수의 경우에는 맨
	 * 앞자리나 001 0이 아닌 수가 나오기 전까지 모두 0이면 001 -> 1로 변하는 마법 01055556666 -> 1055556666
	 * 100000 ~ 189999 사이의 임의의 숫자 6자리 숫자 생성
	 * 
	 * 만약에 5자리 number = (int)(Math.random() * (9000)) + 10000;
	 */
	...
	public MimeMessage createMail(String mail) {
		...
		try {
			MimeMessageHelper helper = new MimeMessageHelper(메세지, true);

			helper.setFrom(senderEmail); // 인증번호를 전송하는 주최자 이메일 주소
			helper.setTo(mail);
	...
	public int sendMail(String mail) {
		...
		try {
			...
			log.info("success email send : " + mail);
		}
