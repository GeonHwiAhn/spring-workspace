...
public class PigService {
	...
	*/
	...
	// 3. 돼지정도 DB에 업로드하기	->	void (Pig)
	// public void uploadPig(Pig pig) { -> 돼지정보 저장할 때 경로나 이름 아무것도 변경할 게 없을 때 사용
	public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		...
		// 만약에 저장하고자하는 이미지폴더가 존재하지 않을 경우 경로들 모두 생성!
		// exist -> File 자료형에서만 생성
		if (!폴더경로확인.exists()) {
			폴더경로확인.mkdirs(); //mkdir = 폴더 한개 mkdirs = 폴더 여러개
			// mkdir 써도 되긴 하지만 혹시모를 문제를 최소화하기 위해 mkdirs 사용
		}
		
		
		//폴더가 안전하게 생성됐는지 확인을 모두 마치고 지정된 폴더에 사진올리기
		File 사진파일올리기 = new File(이미지저장폴더 + 파일이름);
		String DB에파일경로및파일명전달 = 이미지저장폴더 + 파일이름;
		
		//file.transferTo(사진파일올리기); Try/Catch
		...
		try {
			...
			log.info("Mapper에 Service로 pig insert 전달 성공");
		} catch (IOException e) {
