...
public class LoginController {
	...
	@GetMapping("/myPage")
	public String showMePage(HttpSession session, Model model) {
	 ...
	 * 
	 * Member member = (Member) session.getAttribute("loginSession");
				 └──── session에서 loginSession이라는 변수명에 저장된 로그인 정보를 가져오기
	...
	public String deleteMember(HttpSession session) {
		...
		session.invalidate(); //삭제 후 로그인 없던일로 처리
		return "redirect:/";
	 ...
	 * inptu에서 name , th:field로 작성된 변수명과 무조건 일치 @RequestParam("   ")
	 * 
