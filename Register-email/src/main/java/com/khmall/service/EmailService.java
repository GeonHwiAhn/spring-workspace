...
//@RequiredArgsConstructor @Autowired와 같지만 내부에서 하나씩 해주는것이 아니라 외부에서 전체적으로 설정
//@Autowired spring에서 만듦, @RequiredArgsConstructor lombok에서 만들어서 이름이 구체적임
public class EmailService {
	
	//자바 자체에서 제공하는 이메일 보내기 기능 툴
	// 왜 툴만 제공하냐면 이메일을 전송하는데 어느회사 어느규격 이메일인지 알 수 없기 때문에
	// 이메일을 보낼 때 보내는이 내용 받는이
	private final JavaMailSender javaMailSender;
	
	//이메일을 보낼 이메일 주소값
	private static final String senderEmail = "geonhwiahn@gmail.com"; //이메일을 보낼 담당자 이메일 작성
	 ...
	 * 예를들어 0.0부터 0.9999  사이의 값을 반환하는 등 실수 생성
	 * Math.random()에 (90000) 곱하기를 해주면 0.0부터 8999999.999.. 사이의 값을 반환
	 ...
	 * 		4자리
	 * number = (int)(Math.random() * (900)) + 1000;
	 * */
	...
	public MimeMessage createMail(String mail) {
		...
		// 메세지 보낼 공간 생성
		//메세지.setFrom(senderEmail); //try catch
		try {
			메세지.setFrom(senderEmail); //인증번호를 전송하는 주최자 이메일주소
