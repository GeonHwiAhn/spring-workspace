...
public class ParameterController {
	...
	public String paramMain() {
		...
		// return 할 때 위치 폴더명/파일명 작성
		// param-main.html 템플릿 밑에 파람이라는 폴더 밑에 존대
		// html 파일을 바라볼 때 기본으로 template 이라는 폴더를 바라보고
	...
	public String paramTest1(HttpServletRequest req) {
		...
		//form 밑에 input 에서 name으로 inputName 존재하는값 가져오기 
		//<input type="text" name="inputName">
		
		String inputAddress = req.getParameter("inputAddress");
		//form 밑에 input 에서 name으로 inputAddress 존재하는값 가져오기 
		//<input type="text" name="inputAddress">
		
		//-> 기본으로 가져오는 값이 String 이기 때문에 숫자값인 Int로 변환
		int inputAge = Integer.parseInt(req.getParameter("inputAge"));
		//form 밑에 input 에서 name으로 inputAge 존재하는값 가져오기 
	...
	public String paramTest2(/* RequestParam은 여기에 보통 작성 */
							 /*value, required=true 생략(기본값 true)*/
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		log.info("mem에 대한 정보 가져오기 : " + mem.getMemberName());
