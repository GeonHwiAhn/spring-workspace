...
public class ImageController {
	 ...
	 * }
	 * */
	@GetMapping("/api/v1/img/upload")	//api 1개 get api = select나 보여주기위한 주소 1개
	...
	public String insertImageUpload(@RequestParam("files") MultipartFile[] files, 
									Model model) {
		String uploadDir = "imgDir"; //이미지저장할폴더명 지정
		Arrays.asList(files).forEach(file -> {
			String fileName = StringUtils.cleanPath(Objects.requireNonNull(file.getOriginalFilename()));
			//파일네임 무사히나오는지 확인하고자 한다면 log sysout 사용해서 출력
			//이미지 업로드하는 서비스에 이미지 업로드할수 있도록 전달
			//ImageUploadService.insertImageUpload(uploadDir, fileName, file); -try/catch
			try {
				...
				//paramMap 의경우 Map<String, Object>로 값을 전달해주지만
